...
```{r}
dat3 <- dat %>% 
  select(rue_1, rue_2, date_, x, y, int_no,acc,ln_cti,ln_cli,ln_cri,borough)

dat3 <- dummy_cols(dat3, select_columns = "borough", remove_selected_columns = TRUE)

dat3[, 8:38] <- lapply(dat3[, 8:38], function(x) scale(x, center = TRUE, scale = TRUE))
...
# ajout des poids
manual_sum <- apply(dat3[, 11:38], 1, sum)
dat3 <- dat3 %>% mutate(indice =ln_cti + ln_cli + ln_cri +manual_sum)

ordreacc <- dat3 %>% select(acc,rue_1,rue_2) %>% 
  arrange(desc(acc))

ordreIN <- dat3 %>% select(indice,rue_1,rue_2) %>% 
  arrange(desc(indice))
